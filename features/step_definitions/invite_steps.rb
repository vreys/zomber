...
Given /^мне отправлено приглашение$/ do
  Given %Q{на имя "#{user_name}" отправлено приглашение по адресу "#{user_email}"}
end
...
Given /^на имя "([^\"]*)" отправлено приглашение по адресу "([^\"]*)"$/ do |name, email|
  When %Q{я запускаю программу приглашения}
  When %Q{я ввожу "#{email}"}
  When %Q{я ввожу "#{name}"}
  Then %Q{вывод должен содержать "Приглашение отправлено по адресу <#{email}> (получатель: #{name})"}
...
Given /^есть зарегестрированный пользователь с email "([^\"]*)"$/ do |email|
  name = Faker::Name.name
  
  Given %Q{на имя "#{name}" отправлено приглашение по адресу "#{email}"}
